...
int main (void)
 ...
 // this is  Inizialization of random number generator
 // one and the same seed leads to one and the sequence of numbers
 random_number = rand();
 //https://cplusplus.com/reference/cstdlib/rand/
 printf("The random number is: %d\n",random_number);
 if(random_number % N == 0 )//NB! operations are not executed simulteniosly
 {
...
if(random_number % N ) // it is not obligatory to compare, because finally is observed value
                        // value obtained inside ()
                        // this value could be "pure" 0 - all bits of obtained result are 0
                        // or at least one bit isnt 0
